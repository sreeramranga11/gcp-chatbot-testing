...
module "bucket" {
  ...
  location                    = each.value.location
  storage_class               = each.value.storage_class
  labels                      = merge(each.value.labels, { environment = each.value.environment, deployment_stage = var.deployment_stage })
  public_access_prevention    = each.value.public_access_prevention
  uniform_bucket_level_access = each.value.uniform_bucket_level_access
  versioning                  = each.value.versioning
  lifecycle_rules             = each.value.lifecycle_rules
...
# Example: Filtering buckets by environment and deployment stage (for advanced use cases and multi-layered deployments)
# locals {
#   dev_buckets = [b for b in var.bucket if b.environment == "dev" && b.deployment_stage == "staging"]
#   prod_buckets = [b for b in var.bucket if b.environment == "prod" && b.deployment_stage == "production"]
# }
#
# module "dev_bucket" {
#   source = "gitrepo.dev//terraform-google-cloud-storage"
