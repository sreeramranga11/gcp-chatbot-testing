...
# Example: Filtering buckets by environment (for advanced use cases)
locals {
  target_environment = "staging" # Set this to the desired environment
  env_buckets = [
    for bucket in var.bucket : bucket
    if bucket.environment == local.target_environment
  ]
}

module "env_bucket" {
  source = "gitrepo.dev//terraform-google-cloud-storage"
  for_each = { for bucket in local.env_buckets : bucket.name => bucket }
  name                        = each.key
  project_id                  = each.value.project_id
  location                    = each.value.location
  storage_class               = each.value.storage_class
