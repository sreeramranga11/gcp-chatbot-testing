...
variable "bucket" {
  type = list(object({
    ...
    project_id                  = string
    environment                 = string # Added for multi-environment support
    deployment_stage            = string # Added for multi-layered deployments
    location                    = string
    storage_class               = string
    public_access_prevention    = string
    uniform_bucket_level_access = bool
...
variable "deployment_stage" {
  type        = string
  description = "The deployment stage, used for labeling"
  default     = "production" # Or whatever the default is.  Should also be configured to support dev and staging environments.
}
